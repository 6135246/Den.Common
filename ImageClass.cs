...
namespace Den.Common
{
    ...
    public class ImageClass
    {
        ...
        #region 对图片对象添加文字水印处理方法
        /// <summary>
        /// 对图片对象添加文字水印处理方法
        /// </summary>
        /// <param name="oragionimage">背景图片</param>
        /// <param name="size">字体大小</param>
        /// <param name="letter">水印文字</param>
        /// <param name="color">颜色</param>
        /// <param name="dx">水印X轴位置</param>
        /// <param name="dy">水印Y轴位置</param>
        /// <param name="FontName">字体</param>
        /// <returns>添加了文字水印后的图片</returns>
        /// 
        public Image LetterWatermark(Image oragionimage, int size, string letter, Color color, int dx, int dy, string FontName = "宋体")
        {
            Graphics gs = Graphics.FromImage(oragionimage);
            Brush br = new SolidBrush(color);
            try
            {
                Font font = new Font(FontName, size, FontStyle.Bold);// new Font(System.Configuration.ConfigurationManager.AppSettings["FontType"], size);
                gs.DrawString(letter, font, br, dx, dy);
                return oragionimage;
            }
            catch (Exception ex)
            {
                Den.Common.Unit.WriteErrLogToTxt("LetterWatermark", "对图片对象添加文字水印处理方法出错", ex);
                return null;
            }
            finally
            {
        ...
        #region 对图片对象添加图片水印处理方法
        /// <summary>
        ///  对图片对象添加图片水印处理方法
        /// </summary>
        /// <param name="originalImage">背景图片</param>
        /// <param name="waterImage">水印图片</param>
        /// <param name="dx">水印X轴位置</param>
        /// <param name="dy">水印Y轴位置</param>
        /// <returns>添加了图片水印的背景图片</returns>
        public Image ImageWatermark(Image originalImage, Image waterImage, int dx, int dy)
        {
            Graphics g = Graphics.FromImage(originalImage);
            string newpath = string.Empty;
            try
            {
                g.DrawImage(waterImage, new Rectangle(dx, dy, waterImage.Width, waterImage.Height));
                return originalImage;
            }
            catch (Exception ex)
            {
                Den.Common.Unit.WriteErrLogToTxt("ImageWatermark", "对图片对象添加图片水印处理方法出错", ex);
                return null;
            }
            finally
            {
        ...
        /// 对图片对象创建缩略图
        /// </summary>
        /// <param name="image">需要缩略的图片对象</param>
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式</param>
        /// <returns>缩略处理完之后的图片对象</returns>
        ///  
        public Image MakeThumbnail(Image image, int width, int height, string mode)
        {
            int towidth = width;
            int toheight = height;
            int x = 0;
            ...
            {
                Den.Common.Unit.WriteErrLogToTxt("MakeThumbnail", "对图片对象创建缩略图失败!", ex);
                return null;
            }
            finally
            {
        ...
        #region 正方形转换成圆形图片
        public Image ToCircular(Image img, Rectangle rec, Size size)
        {
        ...
        public static string ImageWatermark(string path, string waterpath, string location)
        {
            ...
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg" || kz_name == ".png")
            {
                ...
                {
                    FilesDelete.DeleteFile(newpath, 1);
                }
            }
            return path;
        }
        ...
        /// <param name="waterpath">水印图片（绝对路径）</param>
        /// <param name="location">水印位置（传送正确的代码）</param>
        public static string ImageWatermark(string path, string waterpath, int dx, int dy)
        {
            string kz_name = Path.GetExtension(path);
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg" || kz_name == ".png")
            {
                ...
                Image waterimg = Image.FromFile(waterpath);
                Graphics g = Graphics.FromImage(img);
                ArrayList loca = GetLocation(dx, dy);
                g.DrawImage(waterimg, new Rectangle(int.Parse(loca[0].ToString()), int.Parse(loca[1].ToString()), waterimg.Width, waterimg.Height));
                waterimg.Dispose();
                g.Dispose();
                string newpath = Path.GetDirectoryName(path) + filename + kz_name;
        ...
        /// 图片水印位置处理方法
        /// </summary>
        /// <param name="location">水印位置(LT,B,CT)</param>
        ...
        /// 图片水印位置处理方法
        /// </summary>
        /// <param name="x">水印x坐标偏移量</param>
        /// <param name="y">水印y坐标偏移量</param>
        /// <param name="img">需要添加水印的图片</param>
        /// <param name="waterimg">水印图片</param>
        private static ArrayList GetLocation(int dx, int dy)
        {
            ArrayList loca = new ArrayList();
            loca.Add(dx);
            loca.Add(dy);
        ...
        /// <param name="letter">水印文字</param>
        /// <param name="color">颜色</param>
        /// <param name="x">水印X偏移</param>
        /// <param name="y">水印Y偏移</param>

        public static string LetterWatermark(string path, int size, string letter, Color color, int dx, int dy)
        {
            #region
